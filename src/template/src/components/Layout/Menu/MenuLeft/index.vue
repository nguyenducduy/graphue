          ...
          <template v-for="item in items">
            <a-menu-item
              ...
              v-if="
                typeof item.children === 'undefined' ||
                  (item.children.length == 0 && accessMenu.some((e) => e.id == item.id))
              "
            >
              <router-link v-if="item.path !== ''" :to="item.path">
                <i v-if="item.icon" :class="[$style.icon, item.icon]"></i>
                <span :class="$style.title">{{ $t(`menu.${item.name}`) }}</span>
              </router-link>
              <span v-else>
                <i v-if="item.icon" :class="[$style.icon, item.icon]"></i>
                <span :class="$style.title">{{ $t(`menu.${item.name}`) }}</span>
            ...
            >
              ...
              <template v-for="child in item.children">
                <a-menu-item :key="`${child.id}-${child.path}`" v-if="accessMenu.some((e) => e.id == child.id)">
                  <router-link v-if="child.path !== ''" :to="child.path">
                    <span :class="$style.title">{{ $t(`menu.${child.name}`) }}</span>
                  </router-link>
                  <span v-else>
                    <span :class="$style.title">{{ $t(`menu.${child.name}`) }}</span>
                  </span>
