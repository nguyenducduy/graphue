    ...
    <Can I="listUser">
      ...
      >
        ...
        <template slot="_avatar" slot-scope="record">
          ...
          />
          <a-avatar v-else size="small" :style="`backgroundColor: ${record.node.status.color}`">{{
            record.node.fullName[0]
          }}</a-avatar>
        </template>
        <template slot="_name" slot-scope="value">{{ value }}</template>
        <a-tag slot="_status" slot-scope="record" :color="record.node.status.color">{{
          record.node.status.text
        }}</a-tag>
        <a-tag slot="_group" slot-scope="record" :color="record.node.group.color">{{
          record.node.group.screenName
        }}</a-tag>
  ...
  get columns() {
    ...
    const columns: any = [
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: sortedInfo.columnKey === 'oauth_provider' && sortedInfo.order,
