...
export function getVariables(context, curPage) {
  ...
  const totalPages: number = Math.floor(
    context.total / context.pagination.pageSize
  );
  ...
  if (
    typeof currentQs["filter"] !== "undefined" &&
    Array.isArray(currentQs["filter"])
  ) {
    let customFilterKey = "";
    let customFilterVal = [];
    ...
    currentQs["filter"].map(filter => {
      ...
      filterObject[customFilterKey].push(
        _convertFilterValue(filterBy, filterValue)
      );
    });
    ...
    typeof currentQs["filter"] !== "undefined" &&
    typeof currentQs["filter"] == "string"
  ) {
    let [filterBy, filterValue] = currentQs["filter"].split(".");
    filterObject[_convertFilterBy(filterBy)] = _convertFilterValue(
      filterBy,
      filterValue
    );
  ...
  if (typeof currentQs["q"] !== "undefined" && currentQs["q"] !== "") {
    ...
    context["searchInInfo"].map(item => {
      let searchContext = {};
      (searchContext[_convertSearchBy(item.field)] =
        "%" + currentQs["q"] + "%"),
        searchObject.push(searchContext);
    });
